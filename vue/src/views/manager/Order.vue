    ...
    <el-col :span="6" v-for="item in data.foodsList" :key="item.id">
      <div class="card">
        ...
        <div style="margin: 5px; color: #000; font-size: 18px;display: flex;align-items: center">
          ...
          <div style="font-weight: bold;color: #c40606">￥{{ item.price }}</div>
        </div>
        <div style="margin: 5px; color: #666">
          <el-tooltip :content="item.descr" placement="top" effect="customized" v-if="item.descr.length >= 20">
...
const addOrder = (foods) => {
  if (data.table.no) {
    ...
    ElMessage.error("您还未选择餐桌，请先选择餐桌")
  }
}
